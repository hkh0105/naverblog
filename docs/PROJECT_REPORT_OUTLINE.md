...
이 단계에서 사용한 기법은 **프로토타입 개발 방법론** 하나이다. 완성형 상용 서비스 전체를 한 번에 만드는 대신, 실제 동작 가능한 Python/Streamlit 앱을 빠르게 구현하고 실행 결과를 보며 기능을 보완하는 방식으로 개발했다. 이 방법은 요구사항이 많은 서비스에서 사용자 화면과 기능 흐름을 빠르게 검증할 수 있다는 장점이 있다.

![구현 방법론](implementation_methodology.png)

| 구현 단계 | 실제 적용 내용 | 산출물 |
|---|---|---|
| 설계 산출물 확인 | 요구사항 명세서, 사용사례 다이어그램, 화면 설계서를 기준으로 구현 범위 확정 | 기능 목록 |
| 모듈 분해 | 글 생성, 키워드 분석, 이미지 생성, 썸네일, 워터마크, 레퍼런스 관리로 분리 | 모듈 구조 |
| Python 구현 | Streamlit UI와 `src/naverblog` 모듈을 연결하여 앱 기능 구현 | 실행 가능한 앱 |
| 데이터/API 연결 | Naver API, LLM API, SQLite/Supabase, 캐시 구조 연결 | 키워드/생성 기록 데이터 |
| 디버깅 | 문법 검사, 샘플 입력 실행, 위험 표현 탐지 확인 | 오류 수정 결과 |
| 배포 | GitHub push 후 Streamlit Cloud 배포 흐름 구성 | 공유 가능한 웹 앱 |

| 파일/모듈 | 구현 역할 |
|---|---|
| `app.py` | 메인 글 생성 화면, 모델 선택, 카테고리 프리셋, 이미지 옵션, 결과 미리보기 |
| `pages/4_키워드_분석.py` | 검색량, 경쟁도, 등급, 포화지수, 트렌드, 인구통계, 스마트블록 분석 |
| `pages/6_썸네일_제작.py` | Fabric.js 기반 드래그 앤 드롭 썸네일 제작 화면 |
| `pages/3_워터마크.py` | 이미지/PDF 워터마크 적용 기능 |
| `src/naverblog/naver_api.py` | 네이버 검색광고, DataLab, 블로그/카페 발행수, 스마트블록 처리 |
| `src/naverblog/pipeline.py` | 스킬 실행, 프롬프트 조립, LLM 호출, HTML 변환, 기록 저장 |
| `scripts/medical_blog_prototype.py` | 교육용 Python 콘솔 프로토타입 |
