...
| 설계 | Goal 기반 설계 루프 | 연주 | 5/15~5/18 | 기능 설계, 화면 흐름, Goal 루프 |
...
### 4.1 설계 기법 선정
이 단계에서 사용한 기법은 **Goal 기반 설계 루프** 하나이다. 목표 상태를 먼저 정하고, 입력-처리-출력-검토 과정을 반복하면서 기능을 구체화하는 방식이다. 이 프로젝트의 목표는 “개원의가 직접 검토한 듯한 신뢰도 높은 병원 블로그 글을 빠르게 만들고 위험 표현을 줄이는 것”이므로 Goal 중심으로 설계하는 것이 적합하다.

![Goal 기반 설계 루프](goal_loop_design.png)

### 4.2 Goal 루프 적용
| 루프 요소 | 설계 내용 |
|---|---|
| Goal | 신뢰도 높은 병원 블로그 글 생성과 의료광고 위험 표현 감소 |
| Input | 지역, 진료과, 증상, 의사 코멘트, 점검할 문장 |
| Process | 제목 추천, 글 구조 생성, 위험 표현 탐지, 대체 표현 제안 |
| Output | 블로그 제목, 글 구성안, 위험 표현 점검 결과, 안전 문구 |
| Review | 의료진이 최종 확인하고 표현을 수정한 뒤 발행 |

### 4.3 기능 구조
1. 키워드 입력 모듈: 지역·진료과·증상 입력값 정리
2. 제목 추천 모듈: 검색 의도를 반영한 제목 생성
3. 글 구조 생성 모듈: 환자 눈높이 구성 생성
4. 위험 표현 점검 모듈: 금지·주의 표현 탐지
5. 결과 출력 모듈: 글 기획안과 점검 결과 출력
