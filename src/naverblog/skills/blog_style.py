"""Blog style skill based on byhur99 Bobo blog references.

Style data is stored in the app DB and can be edited in the UI.
When older clinic defaults are present, they are replaced with the Bobo defaults.
"""
...
def _is_legacy_clinic_style(key: str, content: str | None) -> bool:
    if not content:
        return False
    if key == "common":
        return "메디블로그 AI 병원 콘텐츠" in content or "의료광고" in content
    return key in LEGACY_CLINIC_STYLE_KEYS


def seed_default_styles(db) -> None:
    """Seed Bobo defaults and remove old clinic defaults when encountered."""
    common = db.get_blog_style("common")
    if common is None or _is_legacy_clinic_style("common", common):
    ...
    for cat_name, style_text in list(db.list_blog_styles().items()):
        if _is_legacy_clinic_style(cat_name, style_text):
            db.delete_blog_style(cat_name)
...
def get_available_categories(db) -> list[str]:
    ...
    default_order = [key for key in DEFAULT_CATEGORY_STYLES if key in styles]
    custom = sorted(key for key in styles if key != "common" and key not in DEFAULT_CATEGORY_STYLES)
    return default_order + custom
...
class BlogStyleSkill(SkillBase):
    """Inject Bobo blog writing style into LLM prompts."""
